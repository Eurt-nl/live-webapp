  ...
  <div class="q-pa-md">
    ...
    <div v-else-if="event || isNew" class="q-gutter-md">
      ...
      <div v-if="!isNew" class="row justify-between items-center">
        <div class="text-h4">{{ $customT('events.editEvent') }} - {{ event?.name }}</div>
            ...
            <div class="row q-col-gutter-md">
              ...
              <div class="col-12 col-md-6">
                <q-input
                  ...
                  :label="$customT('events.eventName')"
              ...
              <div class="col-12 col-md-6">
                <q-input
                  ...
                  :label="$customT('events.startDate')"
                  type="datetime-local"
                  :rules="[(val) => !!val || $customT('validation.required')]"
                  outlined
                  stack-label
              ...
              <div class="col-12 col-md-6">
                <q-input
                  ...
                  :label="$customT('events.maxParticipants')"
                  type="number"
              ...
              <div class="col-12">
                <q-input
                  ...
                  :label="$customT('events.description')"
                  type="textarea"
                  outlined
                  autogrow
                />
              ...
              <div class="col-12">
                <q-select
                  ...
                  option-label="name"
                  option-value="id"
                  :label="$customT('events.moderators')"
                  multiple
                  outlined
                  use-chips
                />
    ...
    <div v-else class="text-center">
      <div class="text-h6">{{ $customT('events.eventNotFound') }}</div>
      <q-btn flat color="primary" :label="$customT('events.back')" @click="router.back()" />
    </div>
  </div>
</template>
...
const loadEvent = async () => {
  try {
    ...
    debug('Error loading event:', error);
    $q.notify({
      color: 'negative',
...
const loadCourses = async () => {
  try {
    ...
    // Foutmelding bij mislukken
    debug('Error loading courses:', error);
    $q.notify({
      color: 'negative',
      message: $customT('events.loadError', { error: error.message }),
      icon: 'error',
...
const loadUsers = async () => {
  try {
    ...
    // Foutmelding bij mislukken
    debug('Error loading users:', error);
    $q.notify({
      color: 'negative',
      message: $customT('notifications.loadUsersError'),
      icon: 'error',
    });
  }
};
...
const saveEvent = async () => {
  try {
    ...
    // Foutmelding bij mislukken
    debug('Error saving event:', error);
    $q.notify({
      color: 'negative',
      message: $customT('notifications.saveEventError'),
      icon: 'error',
    });
  } finally {
    saving.value = false;
