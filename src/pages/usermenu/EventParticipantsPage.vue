<template>
  <div class="q-pa-md">
        ...
        <q-card-section>
          <div class="row items-center q-mb-md">
            <div class="col text-subtitle2">
              {{ $customT('participants.currentParticipants') }} ({{ registrations.length }}/{{
                event.max_players
              }})
            </div>
            <div class="col-auto">
              <q-btn
                v-if="isModerator"
                color="primary"
                :label="$customT('participants.addPlayer')"
        ...
        <q-card style="min-width: 320px; max-width: 90vw">
          <q-card-section>
            ...
            <q-select
              ...
              option-label="name"
              option-value="id"
              :label="$customT('participants.player')"
              emit-value
              map-options
              outlined
            ...
            <q-select
              ...
              option-label="name"
              option-value="id"
              :label="$customT('participants.category')"
              emit-value
              map-options
              outlined
              class="q-mt-sm"
            ...
            <q-select
              ...
              option-label="name"
              option-value="id"
              :label="$customT('participants.status')"
              emit-value
              map-options
              outlined
              class="q-mt-sm"
            ...
            <q-input
              ...
              :label="$customT('participants.note')"
              type="textarea"
              outlined
              class="q-mt-sm"
            />
          ...
          <q-card-actions align="right">
            <q-btn flat :label="$customT('participants.cancel')" color="primary" v-close-popup />
            <q-btn
              flat
              :label="isEdit ? $customT('participants.save') : $customT('participants.add')"
              color="primary"
    ...
    <div v-else class="text-center">
      ...
      <q-btn flat color="primary" :label="$customT('participants.back')" @click="router.back()" />
    </div>
  </div>
</template>
...
const loadCategories = async () => {
  try {
    ...
    userCategories.value = userCatResult.items.map((item: Record<string, unknown>) => ({
      id: typeof item.id === 'string' && item.id !== '[object Object]' ? item.id : '',
      name: typeof item.name === 'string' && item.name !== '[object Object]' ? item.name : '',
      cat_type:
        typeof item.cat_type === 'string' && item.cat_type !== '[object Object]'
          ? item.cat_type
          : '',
    ...
    // Cast de PocketBase resultaten naar Category type
    registrationStatuses.value = regStatusResult.items.map((item: Record<string, unknown>) => ({
      id: typeof item.id === 'string' && item.id !== '[object Object]' ? item.id : '',
      name: typeof item.name === 'string' && item.name !== '[object Object]' ? item.name : '',
      cat_type:
        typeof item.cat_type === 'string' && item.cat_type !== '[object Object]'
    ...
    $q.notify({
      color: 'negative',
      message: $customT('notifications.loadCategoriesError'),
      icon: 'error',
    });
  }
};
...
async function saveParticipant() {
  try {
    if (isEdit.value) {
      ...
      $q.notify({
        color: 'positive',
        message: $customT('notifications.registrationUpdated'),
        icon: 'check',
      });
    } else {
      if (registrations.value.some((r) => r.user === addForm.value.user)) {
        $q.notify({
          color: 'warning',
          message: $customT('notifications.playerAlreadyEnrolled'),
          icon: 'warning',
        });
        return;
      }
    ...
    $q.notify({ color: 'negative', message: $customT('notifications.saveError'), icon: 'error' });
  }
}
...
const removeParticipant = async (userId: string) => {
  try {
    ...
    $q.notify({
      color: 'negative',
      message: $customT('notifications.deleteParticipantError'),
      icon: 'error',
    });
  }
};
