          ...
          <q-card class="cursor-pointer my-card" @click="editRound(round)" flat>
            <q-card-section>
              <div class="row items-center">
                <div class="col">
                  <div class="row items-center">
                    <div class="text-subtitle1">Ronde {{ String(round.round_number) }}</div>
                  </div>
...
const validateDateTime = (dateTimeStr: string, roundNumber: number) => {
  ...
  for (const round of lowerRounds) {
    if (new Date(round.date_time_event_round) > roundDate) {
      return `Datum en/of tijd kan niet voor ronde ${String(round.round_number || '')} liggen`;
    }
  }
  for (const round of higherRounds) {
    if (new Date(round.date_time_event_round) < roundDate) {
      return `Datum en/of tijd kan niet na ronde ${String(round.round_number || '')} liggen`;
    }
  }
  return true;
};
  ...
  try {
    ...
    for (const reg of regsResult) {
      try {
        ...
        console.error('Fout bij aanmaken ronde voor registratie', reg.id);
      }
    }
    if (createdCount > 0) {
...
const confirmStartRound = async () => {
  ...
  if (catId === SKINS_ID || catId === COMPETITIE_ID) {
    ...
    void linkConfirmedRegistrations(roundToStart.value);
    roundToStart.value = null;
  }
};
